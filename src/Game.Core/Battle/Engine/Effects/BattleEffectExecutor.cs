		...
		switch (effect)
		{
			...
			case SummonCombatantBattleHookEffectDefinition summon:
				...
				if (impactedPositions!=null) {
						...
						var battleJoinCombatant = engine.StateFactory.SpawnCombatant(source, state, summon.characterIds, impactedPositions);
					...
					BattleEngine.AddMessage(state, battleEvent);
				}
				break;
